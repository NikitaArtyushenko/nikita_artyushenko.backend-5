...
if ($_SERVER['REQUEST_METHOD'] == 'GET') {
  ...
  $values['gender'] = empty($_COOKIE['year_value']) ? '' : $_COOKIE['year_value'];
  $values['limbs'] = empty($_COOKIE['year_value']) ? '' : $_COOKIE['year_value'];
  $values['abilities'] = empty($_COOKIE['year_value']) ? '' : $_COOKIE['year_value'];
  ...
  foreach ($_POST['abilities'] as $ability){
    if ($ability != "Бессмертие" && $ability != "Прохождение сквозь стены" && $ability != "Левитация"){
      ...
      setcookie('ab_value', $_POST['abilities'], time() + 365 * 24 * 60 * 60);
    }
  }
